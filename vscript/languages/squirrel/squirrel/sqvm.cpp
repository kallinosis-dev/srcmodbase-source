	...
	case OT_WEAKREF:
		{
			...
			{
				SQ_VALIDATE_REF_COUNT( o._unVal.pWeakRef->_obj._unVal.pRefCounted );
...
common_call:
			...
			case _OP_RETURN:
                ...
                if( _qs_fn && --_qs_cnt<0 ){
query_suspend:                
                    _qs_cnt = SQ_QUERY_COUNT_START;
                    int r = _qs_fn(this);
                    if( r==SQ_QUERY_BREAK ){
						Raise_Error(_SC("Script terminated by SQQuerySuspend"), NULL);
						SQ_THROW();
                    }
                    else if( r==SQ_QUERY_SUSPEND ){
                        // This code path is not tested very much 
                        _suspended = SQTrue;
                        _suspended_target = ct_target;
                        _suspended_root = ci->_root;
                        _suspended_traps = traps;
			...
			case _OP_JMP: ci->_ip += (sarg1); 
                if( _qs_fn && --_qs_cnt<0 ) goto query_suspend;
                continue;
			case _OP_JNZ: if(!IsFalse(STK(arg0))) ci->_ip+=(sarg1); 
                if( _qs_fn && --_qs_cnt<0 ) goto query_suspend;
                continue;
			case _OP_JZ: if(IsFalse(STK(arg0))) ci->_ip+=(sarg1); 
                if( _qs_fn && --_qs_cnt<0 ) goto query_suspend;
                continue;
