...
{
	CProjectGenerator_PS3 *pNew = new CProjectGenerator_PS3();
...
CProjectGenerator_PS3::CProjectGenerator_PS3()
{
	...
	m_pVCProjGenerator->SetupGeneratorDefinition( "ps3.def", s_PS3PropertyNames );
...
bool CProjectGenerator_PS3::WritePreBuildEventTool( CPreBuildEventTool *pPreBuildEventTool )
{
	...
	m_XMLWriter.AddNodeProperty( "Name=\"VCPreBuildEventTool\"" );
	...
	for ( int i = 0; i < pPreBuildEventTool->m_PropertyStates.m_Properties.Count(); i++ )
	{
		...
		case PS3_PREBUILDEVENT_CommandLine:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "CommandLine=\"%s\"", m_XMLWriter.FixupXMLString( pPreBuildEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ).Get() ) );
			break;

		case PS3_PREBUILDEVENT_Description:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "Description=\"%s\"", pPreBuildEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_PREBUILDEVENT_ExcludedFromBuild:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "ExcludedFromBuild=\"%s\"", BoolStringToTrueFalseString( pPreBuildEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) ) );
			break;
		}
	}
...
bool CProjectGenerator_PS3::WriteCustomBuildTool( CCustomBuildTool *pCustomBuildTool )
{
	...
	m_XMLWriter.AddNodeProperty( "Name=\"VCCustomBuildTool\"" );
	...
	for ( int i = 0; i < pCustomBuildTool->m_PropertyStates.m_Properties.Count(); i++ )
	{
		...
		case PS3_CUSTOMBUILDSTEP_CommandLine:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "CommandLine=\"%s\"", m_XMLWriter.FixupXMLString( pCustomBuildTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ).Get() ) );
			break;

		case PS3_CUSTOMBUILDSTEP_Description:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "Description=\"%s\"", pCustomBuildTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_CUSTOMBUILDSTEP_Outputs:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "Outputs=\"%s\"", pCustomBuildTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_CUSTOMBUILDSTEP_AdditionalDependencies:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "AdditionalDependencies=\"%s\"", pCustomBuildTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;
		}
	}
...
bool CProjectGenerator_PS3::WriteSNCCompilerTool( CCompilerTool *pCompilerTool )
{
	...
	m_XMLWriter.AddNodeProperty( "Name=\"VCCLCompilerTool\"" );
	...
	for ( int i = 0; i < pCompilerTool->m_PropertyStates.m_Properties.Count(); i++ )
	{
		...
		case PS3_SNCCOMPILER_AdditionalIncludeDirectories:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "AdditionalIncludeDirectories=\"%s\"", pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_SNCCOMPILER_PreprocessorDefinitions:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "PreprocessorDefinitions=\"%s\"", pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_SNCCOMPILER_ForceIncludes:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "ForcedIncludeFiles=\"%s\"", pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;
		...
		case PS3_SNCCOMPILER_DisableSpecificWarnings:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "DisableSpecificWarnings=\"%s\"", pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_SNCCOMPILER_ObjectFileName:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "ObjectFile=\"%s\"", pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;
		...
		case PS3_SNCCOMPILER_CreateUsePrecompiledHeader:
			if ( nOrdinalValue == 1 )
			{
				additionalOptions += CFmtStrMax( "--create_pch=&quot;%s&quot; ", pCompilerTool->m_PropertyStates.GetProperty( PS3_SNCCOMPILER_PrecompiledHeaderFile )->m_StringValue.Get() );
			}
			else if ( nOrdinalValue == 2 )
			{
			...
			{
				additionalOptions += CFmtStrMax( "--use_pch=&quot;%s&quot; ", pCompilerTool->m_PropertyStates.GetProperty( PS3_SNCCOMPILER_PrecompiledHeaderFile )->m_StringValue.Get() );
			}
			break;
	...
	{
		m_XMLWriter.AddNodeProperty( CFmtStrMax( "AdditionalOptions=\"%s\"", additionalOptions.Get() ) );
	}
...
bool CProjectGenerator_PS3::WriteGCCCompilerTool( CCompilerTool *pCompilerTool )
{
	...
	for ( int i = 0; i < pCompilerTool->m_PropertyStates.m_Properties.Count(); i++ )
	{
		...
		case PS3_GCCCOMPILER_AdditionalIncludeDirectories:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "AdditionalIncludeDirectories=\"%s\"", m_XMLWriter.FixupXMLString( pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ).Get() ) );
			break;
...
bool CProjectGenerator_PS3::WritePreLinkEventTool( CPreLinkEventTool *pPreLinkEventTool )
{
	...
	m_XMLWriter.AddNodeProperty( "Name=\"VCPreLinkEventTool\"" );
	...
	for ( int i = 0; i < pPreLinkEventTool->m_PropertyStates.m_Properties.Count(); i++ )
	{
		...
		case PS3_PRELINKEVENT_CommandLine:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "CommandLine=\"%s\"", m_XMLWriter.FixupXMLString( pPreLinkEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ).Get() ) );
			break;

		case PS3_PRELINKEVENT_Description:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "Description=\"%s\"", pPreLinkEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_PRELINKEVENT_ExcludedFromBuild:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "ExcludedFromBuild=\"%s\"", BoolStringToTrueFalseString( pPreLinkEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) ) );
			break;
		}
	}
...
bool CProjectGenerator_PS3::WriteSNCLinkerTool( CLinkerTool *pLinkerTool )
{
	...
	m_XMLWriter.AddNodeProperty( "Name=\"VCLinkerTool\"" );
	...
	for ( int i = 0; i < pLinkerTool->m_PropertyStates.m_Properties.Count(); i++ )
	{
		...
		case PS3_SNCLINKER_OutputFile:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "OutputFile=\"%s\"", pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;
		...
		case PS3_SNCLINKER_AdditionalDependencies:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "AdditionalDependencies=\"%s\"", pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_SNCLINKER_AdditionalLibraryDirectories:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "AdditionalLibraryDirectories=\"%s\"", pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_SNCLINKER_IgnoreAllDefaultLibraries:
			if ( nOrdinalValue )
			{
				m_XMLWriter.AddNodeProperty( "IgnoreAllDefaultLibraries=\"true\"" );
			}
			break;
		...
		case PS3_SNCLINKER_ForceSymbolReferences:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "ForceSymbolReferences=\"%s\"", pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;
		...
		case PS3_SNCLINKER_ImportLibrary:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "ImportLibrary=\"%s\"", pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_SNCLINKER_GenerateMapFile:
			if ( nOrdinalValue == 1 )
			{
				additionalOptions += CFmtStrMax( "-Map=&quot;%s&quot; ", pLinkerTool->m_PropertyStates.GetProperty( PS3_SNCLINKER_MapFileName )->m_StringValue.Get() );
			}
			else if ( nOrdinalValue == 2 )
			{
				additionalOptions += CFmtStrMax( "-Map=&quot;%s&quot; -sn-full-map ", pLinkerTool->m_PropertyStates.GetProperty( PS3_SNCLINKER_MapFileName )->m_StringValue.Get() );
			}
			break;

		case PS3_SNCLINKER_MapFileName:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "MapFileName=\"%s\"", pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_SNCLINKER_LinkLibraryDependencies:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "LinkLibraryDependencies=\"%s\"", BoolStringToTrueFalseString( pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) ) );
			break;
...
bool CProjectGenerator_PS3::WriteGCCLinkerTool( CLinkerTool *pLinkerTool )
{
	...
	for ( int i = 0; i < pLinkerTool->m_PropertyStates.m_Properties.Count(); i++ )
	{
		...
		case PS3_GCCLINKER_GenerateMapFile:
			if ( nOrdinalValue == 1 )
			{
				additionalOptions += CFmtStrMax( "-Map=&quot;%s&quot; ", pLinkerTool->m_PropertyStates.GetProperty( PS3_GCCLINKER_MapFileName )->m_StringValue.Get() );
			}
			break;
...
bool CProjectGenerator_PS3::WriteLibrarianTool( CLibrarianTool *pLibrarianTool )
{
	...
	m_XMLWriter.AddNodeProperty( "Name=\"VCLibrarianTool\"" );
	...
	for ( int i = 0; i < pLibrarianTool->m_PropertyStates.m_Properties.Count(); i++ )
	{
		...
		case PS3_LIBRARIAN_OutputFile:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "OutputFile=\"%s\"", pLibrarianTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;
		case PS3_LIBRARIAN_AdditionalDependencies:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "AdditionalDependencies=\"%s\"", pLibrarianTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;
		case PS3_LIBRARIAN_WholeArchive:
			// can't decode, seems broken
			break;
		case PS3_LIBRARIAN_LinkLibraryDependencies:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "LinkLibraryDependencies=\"%s\"", BoolStringToTrueFalseString( pLibrarianTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) ) );
			break;
		}
	}
...
bool CProjectGenerator_PS3::WritePostBuildEventTool( CPostBuildEventTool *pPostBuildEventTool )
{
	...
	m_XMLWriter.AddNodeProperty( "Name=\"VCPostBuildEventTool\"" );
	...
	for ( int i = 0; i < pPostBuildEventTool->m_PropertyStates.m_Properties.Count(); i++ )
	{
		...
		case PS3_POSTBUILDEVENT_CommandLine:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "CommandLine=\"%s\"", m_XMLWriter.FixupXMLString( pPostBuildEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ).Get() ) );
			break;

		case PS3_POSTBUILDEVENT_Description:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "Description=\"%s\"", pPostBuildEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_POSTBUILDEVENT_ExcludedFromBuild:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "ExcludedFromBuild=\"%s\"", BoolStringToTrueFalseString( pPostBuildEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) ) );
			break;
		}
	}
...
bool CProjectGenerator_PS3::WriteConfiguration( CProjectConfiguration *pConfig )
{
	...
	for ( int i = 0; i < pConfig->m_PropertyStates.m_Properties.Count(); i++ )
	{
		...
		case PS3_GENERAL_ConfigurationType:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "ConfigurationType=\"%s\"", pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_GENERAL_ExcludedFromBuild:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "ExcludedFromBuild=\"%s\"", BoolStringToTrueFalseString( pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) ) );
			break;

		case PS3_GENERAL_OutputDirectory:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "OutputDirectory=\"%s\"", pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_GENERAL_IntermediateDirectory:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "IntermediateDirectory=\"%s\"", pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_GENERAL_ExtensionsToDeleteOnClean:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "DeleteExtensionsOnClean=\"%s\"", pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;

		case PS3_GENERAL_BuildLogFile:
			m_XMLWriter.AddNodeProperty( CFmtStrMax( "BuildLogFile=\"%s\"", pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get() ) );
			break;
...
bool CProjectGenerator_PS3::WriteToXML()
{
	...
	m_XMLWriter.AddNodeProperty( CFmtStrMax( "RootNamespace=\"%s\"", m_pVCProjGenerator->GetProjectName() ) );
	...
	m_XMLWriter.AddNodeProperty( "Name=\"win32\"" );
	m_XMLWriter.PopNode();
	m_XMLWriter.PopNode();
