...
void CVCProjGenerator::SetupGeneratorDefinition( const char *pDefinitionName, PropertyName_t *pPropertyNames )
{
	...
	m_pGeneratorDefinition = new CGeneratorDefinition(pDefinitionName, pPropertyNames);
}
...
void CVCProjGenerator::StartProject(CScript* script)
{
...
void CVCProjGenerator::FileExcludedFromBuild( bool bExcluded )
{
	if ( !m_pFileConfig )
	{ 
		logging::SyntaxError( m_Script, "Cannot set %s unless in a $File configuration context", g_pOption_ExcludedFromBuild);
	}
...
bool CVCProjGenerator::StartPropertySection( configKeyword_e eKeyword, bool *pbShouldSkip )
{
	...
	*pbShouldSkip = false;
	...
	case KEYWORD_DEBUGGING:
		if ( m_pFileConfig )
		{
			logging::SyntaxError( m_Script, "%s tool interface for file configuration not implemented.", g_pVPC->KeywordToName( eKeyword ));
		}
...
void CVCProjGenerator::HandleProperty( const char *pPropertyName, const char *pCustomScriptData )
{
	...
	{
		m_Script->PushScript( "HandleProperty custom data", pCustomScriptData, 1, false, false );
	}
	...
	{
		// unknown property
		logging::SyntaxError( m_Script, "Unknown property %s", pPropertyName);
	}

	const char *pToken = m_Script->PeekNextToken( false );
	if ( !pToken || !pToken[0] )
	{
...
bool CVCProjGenerator::GetPropertyValueAsBool( CProjectFile *pProjectFile, const char *pConfigrationName, configKeyword_e configKeyword, const char *pPropertyName, bool bDefaultValue )
{
	...
	return Script_ParseBool( valueString, m_Script );
}
