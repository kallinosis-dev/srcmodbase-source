...
bool CVPC::CheckBinPath(char* pOutBinPath, int outBinPathSize)
{
	...
	if (!bFound)
	{
		log.Error("Failed to determine source directory from current path. Expecting 'vpc_scripts' in source path.");
	}
	...
	{
		log.Error("Executable not running from 'devtools/bin' but from unexpected directory '%s'", szModuleBinPath);
	}
	...
	{
		log.Error("Correct executeable missing, should be at '%s'", pOutBinPath);
	}
...
void CVPC::DetermineSourcePath()
{
	...
	log.Status(Color(0, 255, 255, 255), "Source Path: %s", m_SourcePath.Get());
}
...
void CVPC::ParseBuildOptions(int argc, char const* const* argv)
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		if (!V_stricmp_fast(pArg, "/mksln"))
		{
			if ((i + 1) >= argc)
			{
				log.Error("/mksln requires a filename after it.");
			}
		...
		else if (!V_stricmp_fast(pArg, "/slnitems"))
		{
			...
			if (i >= argc || argv[i][0] == '+' || argv[i][0] == '-' || argv[i][0] == '/' || argv[i][0] == '*' || argv[i]
				[0] == '@')
			{
				log.Error("/slnitems <solution items filename>.");
			}
		...
		else if (!V_stricmp_fast(pArg, "/slnfolder"))
		{
			if ((i + 1) >= argc)
			{
				log.Error("/slnfolder <folder path with search pattern>.");
			}
		...
		else if (!V_stricmp_fast(pArg, "/source_file_config_filter"))
		{
			...
			if (i >= argc || argv[i][0] == '+' || argv[i][0] == '-' || argv[i][0] == '/' || argv[i][0] == '*' || argv[i]
				[0] == '@')
			{
				log.Error("/source_file_config_filter <filter string>.");
			}
...
{
	CContextualLogger log{&_debugCtx};
	...
	log.VerboseStatus("CRC String: %s\n", m_SupplementalCRCString.String());
...
bool CVPC::RestartFromCorrectLocation(bool* pIsChild)
{
	...
	*pIsChild = bIsRestart;
	...
	if (!CheckBinPath(szBinPath, sizeof(szBinPath)))
	{
		if (bIsRestart)
		{
			log.Error("Cyclical Restart: Tell A Programmer!, Aborting.");
		}
		...
		{
			log.Error("Excessive Arguments: Tell A Programmer!, Aborting.");
		}
		int i;
		for (i = 0; i < m_nArgc; i++)
		{
...
public:
	...
	static void ResolveValue(CMacro* pThis)
	{
		...
		{
			log.Error("%s used before the project type has been determined", pThis->GetFullName());
...
bool CVPC::BuildTargetProjects()
{
	...
	public:
		bool VisitProject(projectIndex_t iProject, const char* pScriptPath) override
		{
			...
			{
				if(!g_pVPC->IsQuietValidSpew() || bProjectDirty)
					log.Status("%s", m_CRCCheckStatusSpew.Get());
				else
					log.VerboseStatus("%s", m_CRCCheckStatusSpew.Get());
			}
	...
	if (!m_TargetProjects.Count())
	{
		log.Error("No recognized project(s) to build.  Use /h or /projects or /groups to spew more info.");
	}
...
void CVPC::FindProjectFromVCPROJ(const char* pScriptNameVCProj, int nMainArgc, char const* const* ppMainArgv)
{
	...
	{
		log.Error("Could not resolve '%s' to any known projects", pScriptNameVCProj);
	}
...
void CVPC::SetMacrosAndConditionals()
{
	...
	for (conditional_t* cnd : conditionals.GetAllDefined(CONDITIONAL_PLATFORM))
	{
		...
		log.Warning("Detected multiple target platforms...Disabling '%s'", cnd->m_Name.String());
...
#error "Unsupported platform."
		...
		if (!pPlatformConditional)
		{
			...
			log.Error("Failed to determine target platform.");
		}
	...
	log.Status(Color(0, 255, 255, 255), "Target Platform: %s", platformName.String());
	...
	if (m_bGenMakeProj &&
		!bHaveMakefileTarget)
	{
		log.Error("/genmakeproj can only be used with make targets like /linuxsteamrt64");
	}
                                        ...
                                        const CUtlVector<CDependency_Project*>& referencedProjects)
{
	...
	{
		log.Warning("Error reading dependencies file %s", dependenciesPath.Get());
		return false;
	}
                                     ...
                                     const CUtlVector<CDependency_Project*>& referencedProjects)
{
	...
	{
		log.Warning("Error opening solution dependencies file (%s) for write", dependenciesPath.Get());
		return;
	}
                       ...
                       CProjectDependencyGraph& dependencyGraph)
{
	...
	if (!VPC_AreProjectDependenciesSupportedForThisTargetPlatform())
		log.Error("MKSLN not supported for %s yet!", conditionals.GetTargetPlatformName());

	if (!pSolutionGenerator)
		log.Error("No solution generator exists for this platform.");
	...
	{
		log.Status("Keeping current solution.");
		return;
	}
	...
	log.Status("Updating solution dependencies file...");
...
void CVPC::DetermineSolutionGenerator()
{
	...
	if (bUseMakefile)
	{
		log.Status(Color(0, 255, 255, 255), "Using Makefile generator.");
		...
		if (conditionals.IsDefined("GENERATE_MAKEFILE_VCXPROJ"))
		{
			...
			log.Status(Color(0, 255, 255, 255), "Generating Makefile for Visual Studio %s", pVSName);
	...
	{
		{
			if (m_bUse2022)
			{
				log.Status(Color(0, 255, 255, 255), "Generating for Visual Studio 2022.");
			}
			else if (m_bUse2015)
			{
				log.Status(Color(0, 255, 255, 255), "Generating for Visual Studio 2015.");
			}
			else if (m_bUse2013)
			{
				log.Status(Color(0, 255, 255, 255), "Generating for Visual Studio 2013.");
			}
			else if (m_bUse2012)
			{
				log.Status(Color(0, 255, 255, 255), "Generating for Visual Studio 2012.");
			}
			else
			{
				log.Status(Color(0, 255, 255, 255), "Generating for Visual Studio 2010.");
			}
...
const char* CVPC::BuildTempGroupScript(const char* pVPCScriptName)
{
	...
	if (!_mktemp(tmpName))
	{
		log.Error("Could not generate temp name. Tell a Programmer.\n");
	}
	...
	if (!fp)
	{
		log.Error("Could not open temp file '%s'. Tell a Programmer.\n", m_TempGroupScriptFilename.Get());
	}
	...
	if (!pVPCFilename)
	{
		log.Error("Script %s is not in source path %s\n", vpcScriptFilename.Get(), m_SourcePath.Get());
	}
...
int CVPC::ProcessCommandLine()
{
	...
	if (bScriptIsVPC)
	{
		log.Warning("*** Using Explicit Local VPC Specification Override ***");
