...
static ConVar snd_prefetch_common("snd_prefetch_common", "1", FCVAR_RELEASE,
                                  "Prefetch common sounds from directories specified in " SOUND_PREFETCH_FILE);

ConVar force_audio_english("force_audio_english", "0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_GAMECONSOLE,
...
bool CAudioSourceWave::GetXboxAudioStartupData()
{
	...
	if (!g_pFullFileSystem->RelativePathToFullPath(fileName, "GAME", szFullName, sizeof(szFullName),
	                                               GetAudioPathFilter(), &pathType))
	{
	...
	else
	{
		...
		{
			// get optional data
			if (!g_pFullFileSystem->ReadFile(szFullName, "GAME", fileBuffer, pHeader->staticDataSize,
			                                 sizeof(xwvHeader_t)))
			{
				return false;
			}
		...
		case WAVE_FMT:
			{
				if (walk.ChunkSize() <= sizeof(formatBuffer))
				{
					...
					{
						Warning("Unsupported %d-bit wave file %s\n", (int)((WAVEFORMATEX*)formatBuffer)->wBitsPerSample,
						        pName);
					}
				}
			}
...
void CAudioSourceWave::ParseSamplerChunk(IterateRIFF& walk)
{
	...
	if (LittleLong(samplerChunk.cSampleLoops) > 0)
	{
		...
		{
			char nameBuf[MAX_PATH];
			Msg("Unknown sampler chunk type %d on %s\n", LittleLong(samplerChunk.Loops[0].dwType),
...
public:
	...
	void ParseChunk(IterateRIFF& walk, int chunkName) override;
...
private:
	CAudioSourceMemWave(const CAudioSourceMemWave&) = delete; // not implemented, not accessible
};
...
//-----------------------------------------------------------------------------
CAudioMixer* CAudioSourceMemWave::CreateMixer(int initialStreamPosition, int skipInitialSamples,
                                              bool bUpdateDelayForChoreo, SoundError& soundError
#ifdef WITH_PHONON
...
)
{
	...
	CAudioMixer* pMixer = CreateWaveMixer(
#ifdef WITH_PHONON
		CreateWaveDataHRTF(
#endif
			CreateWaveDataMemory(*this)
#ifdef WITH_PHONON
			, pHRTFVector)
#endif
		, m_format,
...
//-----------------------------------------------------------------------------
int CAudioSourceMemWave::GetOutputData(void** pData, int64 samplePosition, int sampleCount,
	...
	if (IsPC() || !IsGameConsole())
	{
		...
		if (!pData)
		{
			...
			Error("CAudioSourceMemWave (%s): GetDataPointer() failed.",
			      m_pSfx ? m_pSfx->GetFileName(nameBuf, sizeof(nameBuf)) : "m_pSfx = NULL");
		}
...
int CAudioSourceMemWave::GetCacheStatus(void)
{
	...
	if (IsPC() || !IsGameConsole())
	{
		...
		{
			char nameBuf[MAX_PATH];
			wavedatacache->RestartDataLoad(&m_hCache, m_pSfx->GetFileName(nameBuf, sizeof(nameBuf)), m_dataSize,
			                               m_dataStart);
		}
		if (bCompleted)
			return AUDIO_IS_LOADED;
...
void CAudioSourceMemWave::CacheLoad(void)
{
	...
	if (IsPC())
	{
		...
		m_hCache = wavedatacache->
			AsyncLoadCache(m_pSfx->GetFileName(nameBuf, sizeof(nameBuf)), m_dataSize, m_dataStart);
	}
	...
	if (IsGameConsole())
	{
		if (m_hStream == INVALID_STREAM_HANDLE)
		{
			...
			SoundError error;
			m_hStream = wavedatacache->OpenStreamedLoad(pFilename, m_dataSize, m_dataStart, 0, -1, m_dataSize, 1,
			                                            streamFlags, error);
...
public:
	...
	~CAudioSourceStreamWave() override;

	CAudioMixer* CreateMixer(int initialStreamPosition, int skipInitialSamples, bool bUpdateDelayForChoreo,
	...
	int SampleToStreamPosition(int samplePosition) override;
	int StreamToSamplePosition(int streamPosition) override;
...
//-----------------------------------------------------------------------------
CAudioMixer* CAudioSourceStreamWave::CreateMixer(int initialStreamPosition, int skipInitialSamples,
                                                 bool bUpdateDelayForChoreo, SoundError& soundError
#ifdef WITH_PHONON
	, hrtf_info_t* pHRTFVec
#endif
)
{
	...
	IWaveData* pWaveData =
		...
		CreateWaveDataHRTF(
#endif
			CreateWaveDataStream(*this, static_cast<IWaveStreamSource*>(this),
			                     pFileName, m_dataStart, m_dataSize, m_pSfx,
			                     initialStreamPosition, skipInitialSamples,
			                     soundError)
#ifdef WITH_PHONON
			, pHRTFVec)
#endif
	...
	if (pWaveData)
	{
		CAudioMixer* pMixer = CreateWaveMixer(pWaveData, m_format,
#ifdef WITH_PHONON
			pHRTFVec ? 2 :
#endif
		                                      m_channels,
		                                      m_bits, initialStreamPosition, skipInitialSamples, bUpdateDelayForChoreo
		);
		if (pMixer)
		{
			ReferenceAdd(pMixer);
...
//-----------------------------------------------------------------------------
int CAudioSourceStreamWave::GetOutputData(void** pData, int64 samplePosition, int sampleCount,
                                          char copyBuf[AUDIOSOURCE_COPYBUF_SIZE])
{
#if !defined( _GAMECONSOLE )
	...
	if (GetType() == WAVE_FORMAT_PCM)
	{
		...
		if (m_hWaveFileAccess != FILESYSTEM_INVALID_HANDLE)
		{
			g_pFullFileSystem->Seek(m_hWaveFileAccess, m_dataStart + samplePosition * SampleSize(),
...
public:
	...
	bool Init(unsigned int memSize) override;
	void Shutdown() override;

	void LevelInit(char const* mapname) override;
	void LevelShutdown() override;

	CAudioSourceCachedInfo* GetInfo(int audiosourcetype, bool soundisprecached, CSfxTable* sfx) override;
	CAudioSourceCachedInfo* GetInfoByName(const char* soundName) override;
	void RebuildCacheEntry(int audiosourcetype, bool soundisprecached, CSfxTable* sfx) override;
...
private:
	...
	void RecursiveBuildSoundList(CUtlRBTree<FileNameHandle_t, int>& other, const char* pCurrentLanguage,
	                             const char* pCurrentDir, const char* pPathID);
	CacheType_t* BuildCacheFromList(char const* cachename, CUtlRBTree<FileNameHandle_t, int>& list,
...
CAudioSourceCache::CacheType_t* CAudioSourceCache::AllocAudioCache(char const* cachename, bool bNeverCheckDisk)
{
	...
	CacheType_t* cache = new CacheType_t(CFmtStr("%s%s", m_szMODPath.String(), cachename), AUDIOSOURCE_CACHE_VERSION,
	                                     nullptr, UTL_CACHED_FILE_USE_FILESIZE, bNeverCheckDisk);
	return cache;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
CAudioSourceCache::CacheType_t* CAudioSourceCache::LookUpCacheEntry(const char* fn, int audiosourcetype,
...
bool CAudioSourceCache::BuildMasterPrecachedSoundsCache()
{
	...
	char fn[512];
	Q_snprintf(fn, sizeof(fn), "%s/%s%s.cache", AUDIOSOURCE_CACHE_ROOTDIR, MASTER_CACHE_NAME,
	           m_szCurrentLanguage.String());
...
//-----------------------------------------------------------------------------
void CAudioSourceCache::RecursiveBuildSoundList(CUtlRBTree<FileNameHandle_t, int>& root, const char* pLanguage,
                                                const char* pCurrentDir, const char* pathID)
{
	FileFindHandle_t fh;
	FileNameHandle_t handle;
		...
		do
		{
			if (*fn != '.')
			{
				...
				else
				{
					...
					if ((!Q_stricmp(ext, "wav") || !Q_stricmp(ext, "mp3")) && !Q_stristr(fn, ".360.") && !Q_stristr(
						fn, ".ps3."))
					{
						char relative[512];
...
//-----------------------------------------------------------------------------
CAudioSourceCache::CacheType_t* CAudioSourceCache::BuildCacheFromList(char const* cachename,
                                                                      CUtlRBTree<FileNameHandle_t, int>& soundlist,
