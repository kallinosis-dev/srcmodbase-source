...
model_t *g_pSimpleWorldModel = nullptr;
model_t *g_pSimpleWorldModelWater = nullptr;
...
struct ViewWeaponEntry_t
{
	ViewWeaponEntry_t( bool bIsViewModel )
	{
		...
		m_hAsyncVTXControl = nullptr;
		m_hAsyncVVDControl = nullptr;
	}
...
static worldbrushdata_t	*s_pMap = nullptr;
...
void CMapLoadHelper::Init( model_t *pMapModel, const char *pPathName )
{
	...
	s_MapFileHandle = g_pFileSystem->OpenEx( szNameOnDisk, "rb", IsGameConsole() ? FSOPEN_NEVERINPACK : 0, IsGameConsole() ? "GAME" : nullptr);
...
{
	{
		...
		if ( !lh.LumpSize() )
		{
			lh.GetMap()->m_pSurfaceBrushes = nullptr;
			lh.GetMap()->m_pSurfaceBrushList = nullptr;
			return;
		}
...
{
	lh.GetMap()->shadowzbuffers = nullptr;
	if ( !lh.LumpSize() )
	{
		lh.GetMap()->numworldlights = 0;
		lh.GetMap()->worldlights = nullptr;
		return;
	}
...
void Mod_LoadOcclusion( void )
{
	...
	b->occluderpolys = nullptr;
	b->numoccludervertindices = 0;
	b->occludervertindices = nullptr;
...
void Mod_LoadNodes( void )
{
	...
	Mod_SetParent (lh.GetMap()->nodes, nullptr);	// sets nodes and leafs
...
void Mod_LoadLeafs_Version_1( CMapLoadHelper &lh, CMapLoadHelper &ambientLightingLump, CMapLoadHelper &ambientLightingTable )
{
	...
	if ( ambientLightingLump.LumpVersion() != LUMP_LEAF_AMBIENT_LIGHTING_VERSION || ambientLightingTable.LumpSize() == 0 )
	{
		...
		CompressedLightCube *inLightCubes = nullptr;
...
void Mod_LoadLeafMinDistToWater( void )
{
	...
	if( !foundOne || lh.LumpSize() == 0 || !g_pMaterialSystemHardwareConfig )
	{
		...
		lh.GetMap()->m_LeafMinDistToWater = nullptr;
	}
	else
	{
		int		count;
...
bool Mod_LoadGameLump( int lumpId, void *pOutBuffer, int size )
{
	...
	else
	{
		...
		FileHandle_t fileHandle = g_pFileSystem->OpenEx( szNameOnDisk, "rb", IsGameConsole() ? FSOPEN_NEVERINPACK : 0, IsGameConsole() ? "GAME" : nullptr);
...
void CMDLCacheNotify::OnCombinerPreCache( MDLHandle_t OldHandle, MDLHandle_t NewHandle )
{
	...
	g_pMDLCache->SetUserData( OldHandle, nullptr);
...
{
	g_pMDLCache->SetCacheNotify(nullptr);
}
...
model_t *CModelLoader::FindModel( const char *pName )
{
	...
	if ( i == m_Models.InvalidIndex() )
	{
		...
		if ( m_bAllowWeaponModelCache )
		{
			...
			if ( V_stristr( pModel->szPathName, "weapons/v_" ) != nullptr)
			{
			...
			else if ( V_stristr( pModel->szPathName, "weapons/w_" ) != nullptr)
			{
	...
	case mod_brush:
		{
			...
			m_bAllowWorldWeaponEviction = ( V_stristr( m_szBaseName, "training1" ) == nullptr);
...
void CModelLoader::Map_UnloadModel( model_t *mod )
{
	...
	if ( m_worldBrushData.hDispInfos )
	{
		...
		m_worldBrushData.hDispInfos = nullptr;
	}
					...
					if ( m_WeaponModelCache[i]->m_hAsyncVTXControl || m_WeaponModelCache[i]->m_hAsyncVVDControl )
					{
						...
						if ( m_WeaponModelCache[i]->m_hAsyncVTXControl )
						{
							...
							if ( status != FSASYNC_STATUS_PENDING && status != FSASYNC_STATUS_INPROGRESS && status != FSASYNC_STATUS_UNSERVICED )
							{
								...
								m_WeaponModelCache[i]->m_hAsyncVTXControl = nullptr;
							}
						}
						...
						if ( m_WeaponModelCache[i]->m_hAsyncVVDControl )
						{
							...
							if ( status != FSASYNC_STATUS_PENDING && status != FSASYNC_STATUS_INPROGRESS && status != FSASYNC_STATUS_UNSERVICED )
							{
								...
								m_WeaponModelCache[i]->m_hAsyncVVDControl = nullptr;
							}
						}
					}
