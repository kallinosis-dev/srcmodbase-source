...
void IVP_Statistic_Manager::output_statistic() {
    Log_Warning(LOG_HAVOK, "nr_impacts %d  hard_resc %d  resc_after %d  delayed %d  sys_strt %d sys_imps %d unmov %d  coll_check %d  mindists %d  gen_mindists %d ov %d\n",
	...
	if(deb_man->revolve_deb_file) {
	    ...
	    Log_Warning(LOG_HAVOK, "just_opened %s\n",next_name);
	    P_FREE(next_name);
	#endif
	}
    }
