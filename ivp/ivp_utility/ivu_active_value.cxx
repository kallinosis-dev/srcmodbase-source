...
void IVP_U_Active_Value_Manager::insert_active_float(IVP_U_Active_Float *mod)
{
    ...
    if(p_strlen(name)==0){
	Log_Warning(LOG_HAVOK, "insert_active_float: tried to insert active_IVP_FLOAT without name!\n");
	return;
    }
    
    IVP_U_Active_Float *found = (IVP_U_Active_Float *)floats_name_hash->find_active_value(mod);
    if(found){
	Log_Warning(LOG_HAVOK, "insert_active_float: name '%s' already exists in name_hash!\n", name);
	return;
    }
...
void IVP_U_Active_Value_Manager::insert_active_int(IVP_U_Active_Int *mod)
{
    const char *name = mod->get_name();
    if(p_strlen(name)==0){
	Log_Warning(LOG_HAVOK, "insert_active_int: tried to insert active_int without name!\n");
	return;
    }
    
    IVP_U_Active_Int *found = (IVP_U_Active_Int *)ints_name_hash->find_active_value(mod);
    if(found){
	Log_Warning(LOG_HAVOK, "insert_active_int: name '%s' already exists in name_hash!\n", name);
	return;
    }
...
{
    Log_Warning(LOG_HAVOK, "DoubleVal");
...
{
    Log_Warning(LOG_HAVOK, "Sine[F %g, A %g, N %g, ts %g](", frequence, amplitude, null_level, time_shift);
    time_mod->print();
...
{
    Log_Warning(LOG_HAVOK, "Square[F %g, L %g, H %g](", frequence, low_val, high_val);
    time_mod->print();
    Log_Warning(LOG_HAVOK, ")");
    return 0; // f db
}
...
{
    Log_Warning(LOG_HAVOK, "Pulse[F %g, L %g, H %g, %d:%d](", frequence, low_val, high_val, m1, m2);
    time_mod->print();
    Log_Warning(LOG_HAVOK, ")");
    return 0; // f db
}
...
int IVP_U_Active_Add_Multiple::print()
{
    ...
    Log_Warning(LOG_HAVOK, " + %g * (", factor);
    mod1->print();
    Log_Warning(LOG_HAVOK, ")");
...
{
    Log_Warning(LOG_HAVOK, "Limit[%g, %g](", low_val, high_val);
    mod->print();
    Log_Warning(LOG_HAVOK, ")");
    return 0; // for debugger
}
...
{
    Log_Warning(LOG_HAVOK, "TestRange[");
    mod_low_val->print();
    Log_Warning(LOG_HAVOK, ", ");
    mod_high_val->print();
    Log_Warning(LOG_HAVOK, "](");
