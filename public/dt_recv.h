...
public:
	...
	typedef RecvProp	PropType;
				...
				RecvTable();
				RecvTable( RecvProp *pProps, int nProps, char const* pNetTableName );
				~RecvTable();

	void		Construct( RecvProp *pProps, int nProps, char const* pNetTableName );
...
#define BEGIN_RECV_TABLE_NOBASE(className, tableName) \
	...
	{ \
		...
		char const* pRecvTableName = #tableName; \
...
#define RECVINFO(varName)						#varName, offsetof_valve(currentRecvDTClass, varName), sizeof(((currentRecvDTClass*)0)->varName)
#define RECVINFO_NAME(varName, remoteVarName)	#remoteVarName, offsetof_valve(currentRecvDTClass, varName), sizeof(((currentRecvDTClass*)0)->varName)
#define RECVINFO_STRING(varName)				#varName, offsetof_valve(currentRecvDTClass, varName), STRINGBUFSIZE(currentRecvDTClass, varName)
#define RECVINFO_BASECLASS(tableName)			RecvPropDataTable("this", 0, 0, &REFERENCE_RECV_TABLE(tableName))
#define RECVINFO_ARRAY(varName)					#varName, offsetof_valve(currentRecvDTClass, varName), sizeof(((currentRecvDTClass*)0)->varName[0]), sizeof(((currentRecvDTClass*)0)->varName)/sizeof(((currentRecvDTClass*)0)->varName[0])

// Just specify the name and offset. Used for strings and data tables.
#define RECVINFO_NOSIZE(varName)				#varName, offsetof_valve(currentRecvDTClass, varName)
#define RECVINFO_DT(varName)					RECVINFO_NOSIZE(varName)
#define RECVINFO_DTNAME(varName,remoteVarName)	#remoteVarName, offsetof_valve(currentRecvDTClass, varName)
