...
//-----------------------------------------------------------------------------
static bool WriteUserCmdDeltaInt(bf_write *buf, char const* what, int from, int to, int bits = 32)
{
	if ( from != to )
	{
		LogUserCmd( "\t%s %d -> %d\n", what, from, to );
...
static bool WriteUserCmdDeltaShort(bf_write *buf, char const* what, int from, int to)
{
	if ( from != to )
	{
		LogUserCmd( "\t%s %d -> %d\n", what, from, to );
...
static bool WriteUserCmdDeltaFloat(bf_write *buf, char const* what, float from, float to)
{
	if ( from != to )
	{
...
static bool WriteUserCmdDeltaCoord(bf_write *buf, char const* what, float from, float to)
{
	if ( from != to )
	{
		LogUserCmd( "\t%s %2.2f -> %2.2f\n", what, from, to );
...
static bool WriteUserCmdDeltaAngle(bf_write *buf, char const* what, float from, float to, int bits)
{
	if ( from != to )
	{
		LogUserCmd( "\t%s %2.2f -> %2.2f\n", what, from, to );
...
static bool WriteUserCmdDeltaVec3Coord(bf_write *buf, char const* what, const Vector &from, const Vector &to)
{
	if ( from != to )
	{
		LogUserCmd( "\t%s %2.2f -> %2.2f\n", what, from.x, to.x );
