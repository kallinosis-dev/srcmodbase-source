...
{
public:
	CAI_Expresser( CBaseFlex *pOuter = nullptr);
	~CAI_Expresser();

	// --------------------------------
	
	bool Connect( CAI_ExpresserSink *pSink )		{ m_pSink = pSink; return true; }
	bool Disconnect( CAI_ExpresserSink *pSink )	{ m_pSink = nullptr; return true;}

	void TestAllResponses();

	// --------------------------------
	
	bool Speak( AIConcept_t &concept, const char *modifiers = nullptr, char *pszOutResponseChosen = nullptr, size_t bufsize = 0, IRecipientFilter *filter = nullptr);
	bool Speak( AIConcept_t &concept, AI_CriteriaSet *criteria, char *pszOutResponseChosen = nullptr, size_t bufsize = 0, IRecipientFilter *filter = nullptr);
	...
	bool FindResponse( AI_Response &outResponse, AIConcept_t &concept, AI_CriteriaSet *modifiers = nullptr);
	virtual bool SpeakDispatchResponse( AIConcept_t &concept, AI_Response *response, AI_CriteriaSet *criteria, IRecipientFilter *filter = nullptr);
	float GetResponseDuration( AI_Response *response );

	virtual int SpeakRawSentence( const char *pszSentence, float delay, float volume = VOL_NORM, soundlevel_t soundlevel = SNDLVL_TALKING, CBaseEntity *pListener = nullptr);
...
protected:
	...
	bool SpeakRawScene( const char *pszScene, float delay, AI_Response *response, IRecipientFilter *filter = nullptr);
...
public:
	...
	virtual bool 	Speak( AIConcept_t concept, const char *modifiers = nullptr, char *pszOutResponseChosen = nullptr, size_t bufsize = 0, IRecipientFilter *filter = nullptr);
	virtual bool 	Speak( AIConcept_t concept, AI_CriteriaSet *pCriteria, char *pszOutResponseChosen = nullptr, size_t bufsize = 0, IRecipientFilter *filter = nullptr);
	...
	// Find the appropriate response for the given concept. Return false if none found.
	// Fills out the response object that you provide.
	bool FindResponse( AI_Response &outResponse, AIConcept_t &concept, AI_CriteriaSet *criteria = nullptr);

	bool 			SpeakDispatchResponse( AIConcept_t concept, AI_Response *response,  AI_CriteriaSet *criteria = nullptr);
...
protected:
	int 			PlaySentence( const char *pszSentence, float delay, float volume = VOL_NORM, soundlevel_t soundlevel = SNDLVL_TALKING, CBaseEntity *pListener = nullptr);
...
inline bool CAI_ExpresserHost<BASE_NPC>::Speak( AIConcept_t concept, AI_CriteriaSet *pCriteria, char *pszOutResponseChosen /*=NULL*/, size_t bufsize /* = 0 */, IRecipientFilter *filter /* = NULL */ ) 
{
	...
	pExpresser->GatherCriteria( pCriteria, concept, nullptr);
...
public:
	CAI_ExpresserHostWithData( ) : m_pExpresser(nullptr) {};
	...
	{ 
		return BaseClass::CreateComponents() &&	( CreateExpresser() != nullptr);
	}
...
{
public:
	CAI_ExpresserWithFollowup( CBaseFlex *pOuter = nullptr) : CAI_Expresser(pOuter),
...
{
public:
	CMultiplayer_Expresser( CBaseFlex *pOuter = nullptr);
