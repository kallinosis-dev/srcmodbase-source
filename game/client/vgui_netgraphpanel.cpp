...
private:
	...
	typedef struct 
	{
		...
		float flSvFrameStartTimeStdDev;
	} svframerate_t;
...
private:
	...
	float				m_flServerFrameStartTimeStdDeviation;
...
: BaseClass(nullptr, "CNetGraphPanel" )
{
	...
	m_flServerFrameStartTimeStdDeviation = 0;
...
void CNetGraphPanel::DrawTextFields( int graphvalue, int x, int y, int w, netbandwidthgraph_t *graph, cmdinfo_t *cmdinfo )
{
	...
	Q_snprintf( sz, sizeof( sz ), "fps: %5i  var: %4.1f ms  ping: %i ms", (int)(1.0f / m_Framerate), gpGlobals->absoluteframestarttimestddev*1000.0f, (int)(m_AvgLatency*1000.0f) );
	...
	Q_snprintf( sz, sizeof( sz ), "sv:%5.1f %s%4.1f ms   var: %6.3f ms", m_flServerFrameComputationTime*1000.0f,
		...
		m_flServerFramerateStdDeviation * 1000.0f, m_flServerFrameStartTimeStdDeviation * 1000.0f );
...
void CNetGraphPanel::DrawServerType( int xright, int y )
{
	...
	else if ( engine->IsClientLocalToActiveServer() && !bLiveBroadcast )
	{
		psz = "local";
	}
...
void CNetGraphPanel::PaintLineArt( int x, int y, int w, int graphtype, int maxmsgbytes ) 
{
	...
	for (int a=0 ; a<w ; a++)
	{
		...
		if ( !DrawDataSegment( &rcFill, m_Graph[ i ].msgbytes[INetChannelInfo::PAINTMAP], 64, 0, 0 ) )
			continue;
...
{
	netchannel->GetRemoteFramerate( &m_flServerFrameComputationTime, &m_flServerFramerateStdDeviation, &m_flServerFrameStartTimeStdDeviation );
	...
	m_SvFrameRate[ m_nServerFramerateSample % TIMINGS ].flSvFrameStartTimeStdDev = m_flServerFrameStartTimeStdDeviation;
	...
	if ( net_graphholdsvframerate.GetBool() )
	{
		for ( int j = 0; j < TIMINGS; ++ j )
		{
			...
			if ( m_SvFrameRate[ j ].flSvFrameStartTimeStdDev > m_flServerFrameStartTimeStdDeviation )
				m_flServerFrameStartTimeStdDeviation = m_SvFrameRate[ j ].flSvFrameStartTimeStdDev;
		}
	}
}
